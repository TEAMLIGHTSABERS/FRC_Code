...
When GitHub Desktop opens, then it will default to looking at the "main" branch.  Switch it to the "jsmithDev" branch.
...
## Check That "jsmithDev" is Ready to Be Delivered
After you have tested your development code and you feel that the code is good enough for competition, the next step is to check for any changes that have been made on the "main" branch while you were devloping your code.  To do this, make a "pull" request in GitHub Web.  The request is to pull the changes from the "main" branch into your "jsmithDev" Branch.  If there are no changes identified in this pull request, then "jsmithDev" branch is ready to be delivered.  Skip to [Deliver Code to Main Branch](##Deliver_Code_to_Main_Branch).

If the "pull" request from the "main" branch to "jsmithDev" branch shows that changes have been made, then another developer has delivered changes to the competition code since "jsmithDev started implementing and testing. To prevent overwriting delivered changes, all of these changes from this request must be accepted into the "jsmithDev" branch.  When these changes are accepted, it is possible that they will conflict with, or break, the code that is currently in the "jdoeDev" branch.  Check that the code in "jsmithDev" still builds and test it before repeating the this process.  Repeat this process until the "jsmithDev" branch ready to be delivered, as described above.

## Deliver Code to Main Branch
After determining that the "jsmithDev" branch is ready to be delivered, a "pull" request to pull the changes from "jsmithDev" branch into the "main" branch can be made and there should be no conflict.  After you have delivered (merged) the code from the "jsmithDev" Branch into the Main, you should test the "main" code on the robot.  If it works as planned, then the "jsmithDev" Branch can be deleted.

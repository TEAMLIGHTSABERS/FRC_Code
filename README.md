# FRC_Code
This Readme File is for understanding how to use the LIGHTSABERS GitHub for changing FRC Robot Code.  To demonstrate that you can change code in GitHub, pretend that this README file is a Robot source code file and you need to change the a line in this file to make the robot show your name on the driver station display.  In a new GitHub development branch, edit this file and replace the name shown on the "Display Name" line below with your name.  Then commit the change to this branch and pull the changed README file into the 2025 "year" branch so that it can be used on the competition robot.  If that process is not perfectly clear to you, then start reading at [LIGHTSABERS Getting Started with FRC Robot Code](#lightsabers-getting-started-with-frc-robot-code)

Display Name:  Chris Varner
...
Visual Studio will now have a project created in the GitHub Folder called "{New_Project_Name}".
